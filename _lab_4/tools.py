...
#Luu y: Gi ca co logic (Ex: cuoi tuan dat hon, hang cao hon dat hon)
#Sinh vien can doc hieu data de debug test cases
...
def format_vnd(amount: int) -> str:
    """Format số nguyên thành chuỗi tiền tệ VNĐ (VD: 1.450.000đ)"""
...
def search_flights(origin: str, destination: str) -> str:
    ...
    # Thử tra ngược chiều nếu không tìm thấy chiều đi
    if not flights:
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    # Format kết quả
    result = f"Danh sách khách sạn tại {city} (Giá <= {format_vnd(max_price_per_night)}/đêm):\n"
    for idx, h in enumerate(filtered_hotels, 1):
        price_str = format_vnd(h['price_per_night'])
        result += f"{idx}. {h['name']} ({h['stars']} sao) - {h['area']} | Rating: {h['rating']} | Giá: {price_str}/đêm\n"
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        return "Lỗi định dạng expenses. Vui lòng nhập đúng format 'tên_khoản:số_tiền,tên_khoản:số_tiền' (Ví dụ: 'vé_máy_bay:890000,khách_sạn:650000')."
    ...
    # Xử lý cảnh báo âm ngân sách
    if remaining < 0:
        over_budget = abs(remaining)
        result += f"\nVượt ngân sách {format_vnd(over_budget)}! Cần điều chỉnh."
